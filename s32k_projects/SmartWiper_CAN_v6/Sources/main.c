 ...
 * [VERSION] v6 (Refactored & DSP Base)
 * [AUTHOR] sisung
 */
...
#include "lpit1.h"
#include "flexcan_hw.h" // 만든 MCAL 헤더 추가
...
/* [추가] CAN 전송 주기 관리용 변수 */
static volatile uint32_t lastCanTxTime = 0U;
...
void LPIT0_Ch0_IRQHandler(void)
{
	...
	else
	{
		...
		if (controlSource == 0U)
		{
			...
			else if (filteredAdc < 2000U)
			{
				currentMode = MODE_INT;
			}
			else if (filteredAdc < 3500U)
			{
	...
	if (currentMode == MODE_OFF)
	{
		(void) FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 0U,
				FTM_PWM_UPDATE_IN_DUTY_CYCLE, POS_0_DEG, 0U, true);
	...
	else
	{
		...
		if (currentStep == WIPER_IDLE)
		{
			...
			stepCounter = 0U;
			(void) FTM_DRV_UpdatePwmChannel(INST_FLEXTIMER_PWM1, 0U,
					FTM_PWM_UPDATE_IN_DUTY_CYCLE, POS_140_DEG, 0U, true);
		}
		...
		else if (currentStep == WIPER_MOVING_DOWN)
		{
			uint32_t waitTarget =
					(currentMode == MODE_INT) ?
							(moveDuration + INT_WAIT_TIME) : moveDuration;
...
int main(void)
{
	...
	System_Init(); // 여기서 반드시 PTE4/PTE5 핀 설정이 포함되어야 함!
	Sensor_Init();
	Motor_Init();
	Timer_Init();

	/* CAN 초기화 (우리가 만든 함수 호출) */
	FlexCAN0_Init_SDK();
	...
	for (;;)
	{
		...
		/* [ROLE B] 수신 폴링은 무한루프 돌 때마다 계속 물어본다. (타이밍 이슈 해결) */
		if (FLEXCAN_DRV_GetTransferStatus(INST_CANCOM1, 1U) == STATUS_SUCCESS)
		{
			...
			// 덫을 닫았으니, 다시 수신 대기 상태로 만들어 놓음 (비동기 함수 재호출)
...
void Sensor_Init(void)
{
	...
	(void) EDMA_DRV_ConfigSingleBlockTransfer(0U, EDMA_TRANSFER_PERIPH2PERIPH,
			(uint32_t) &(ADC0->R[0]), (uint32_t) &adcValue,
			EDMA_TRANSFER_SIZE_2B, 2U);
...
bool Is_Signal_Stable(uint16_t current)
{
	...
	uint16_t delta =
			(current > lastValue) ?
					(current - lastValue) : (lastValue - current);
