...
void LPIT0_Ch0_IRQHandler(void)
{
	...
	{
		/* [SAFE-STATE 전략]
		 * 센서가 정상이 아닐 때는 모드만 OFF로 바꾸고 '아래 로직을 계속 태워야'
		 * 함. 그래야 [ROLE 3]에서 와이퍼를 0도로 복귀시키는 명령이 실제로
		 * 하드웨어에 전달됨. */
	 ...
	 * PCOR(Port Clear Output Register)는 1을 적어 넣은 비트만 0(Low)으로
	 * 떨어뜨립니다! */
...
void Sensor_Init(void)
{
	 ...
	 * 가변저항의 아날로그 값을 디지털로 변환하고, CPU 개입 없이 DMA가 메모리로
	 * 배달 */
...
void Motor_Init(void)
{
	/* ------------------------------------------------------------------------
	 */
	/* [디버깅용 강제 코드] S32DS GUI가 말을 안 들을 때를 대비한 하드웨어 직접
	 * 제어 */
...
void LED_Init(void)
{
	 ...
	 * 이 핀을 다른 특수 기능이 아닌 일반 입출력(GPIO, MUX=1)으로 쓰겠다고
	 * 선언합니다. */
	 ...
	 * PDDR(Port Data Direction Register)의 해당 비트를 1로 만들면 출력(Output)
	 * 모드가 됩니다. */
	PTD->PDDR |= (1 << 0) | (1 << 15) | (1 << 16);

	/* 4. 초기 상태: 모든 LED 끄기
	 * Active Low이므로, 핀으로 1(High, 5V)을 출력해야 전위차가 없어져서 불이
	 * 꺼집니다. PSOR(Port Set Output Register)는 1을 적어 넣은 비트를 High
	 * 상태로 만듭니다. */
	PTD->PSOR |= (1 << 0) | (1 << 15) | (1 << 16);
}
...
bool Is_Signal_Stable(uint16_t current)
{
	...
	/* [SOPHISTICATED TIP]
	 * 결함 상태에서도 lastValue는 계속 업데이트해줘야 단선 복구 시 오진단을
	 * 막음
	 */
	lastValue = current;
