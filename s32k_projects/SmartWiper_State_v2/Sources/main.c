...
{
    // 인터럽트 플래그를 클리어해야 다음 인터럽트가 발생합니다.
...
//	WDOG->CS &= ~(1U << 7);              // EN(Enable) 비트 해제 (완전히 끄기)
	...
	// UART로 데이터가 들어오면 CPU가 FMSTR_Isr 함수로 즉시 점프하게 만듭니다.
	...
	// 5. 시스템 전체 인터럽트 활성화 (LPIT와 LPUART 동작을 위해 필수)
	...
	for(;;) {
		...
		if (controlSource == 0) {
			...
			/* PC(FreeMASTER) 제어 모드 */
			// PC에서 pcModeRequest 값을 바꾸면 즉시 와이퍼 모드가 변경됩니다.
		...
		if (currentMode != MODE_OFF) {
			...
			{
				if (currentTime - lastTime >= moveDuration) // 자료형 리미트. 오버플로/언더플로.
				{
